...
class calculation_data:
    ...
    def new_dir(self):
        ...
        for frame in range(self.dirframes):
            self.frames.append(frame_data(
                self.nmols, self.ncomps, self.compno))
        return
    ...
    def pull_lipid_data(self, ndir):
        ...
        """
        zdists = pickle.load(
            open("%d/output_zdists_%d.pckl" % (ndir, ndir), 'rb'))
        rg = pickle.load(
            open("%d/output_rg_%d.pckl" % (ndir, ndir), 'rb'))
        p2 = pickle.load(
            open("%d/output_laurp2_%d.pckl" % (ndir, ndir), 'rb'))
        angle = pickle.load(
            open("%d/output_tiltangle_%d.pckl" % (ndir, ndir), 'rb'))
        cosangle = pickle.load(
            open("%d/output_tiltcosangle_%d.pckl" % (ndir, ndir), 'rb'))
        tiltp2 = pickle.load(
            open("%d/output_tiltp2_%d.pckl" % (ndir, ndir), 'rb'))
        for frame in range(self.dirframes):
            self.frames[frame+(ndir-1) *
                        self.dirframes].add_data("zdists", zdists[frame])
            self.frames[frame+(ndir-1) *
                        self.dirframes].add_data("rg", rg[frame])
            self.frames[frame+(ndir-1) *
                        self.dirframes].add_data("laurtilt", p2[frame])
            self.frames[frame+(ndir-1) *
                        self.dirframes].add_data("tiltangle", angle[frame])
            self.frames[frame+(ndir-1) *
                        self.dirframes].add_data("tiltcosangle", cosangle[frame])
            self.frames[frame+(ndir-1) *
                        self.dirframes].add_data("tiltp2", tiltp2[frame])
        return
    ...
    def pull_dir_hyd(self, ndir):
        ...
        for frame in range(self.dirframes):
            ...
            self.frames[frame+(ndir-1) *
                        self.dirframes].add_data("hyd_mol", hyd_data)
        return
    ...
    def pull_voronoi_data(self, ndir):
        ...
        """
        try:
            vor_data = pickle.load(
                open('../voronoi_plots/data/vor_%d.pckl' % ndir, 'rb'))
            ...
            for frame in range(self.dirframes):
                ...
                self.frames[frame_index].add_note(
                    "voronoi: No voronoi data for directory %d" % ndir)
            return
        for frame in range(self.dirframes):
            ...
            l1s, l2s = np.zeros(len(leaf1['molids'])), np.ones(
                len(leaf2['molids']))
    ...
    def hyd_molar_data(self, ndir, comp):
        ...
        """
        hyd = np.loadtxt('%d/hyd_molar_%d.dat' %
                         (ndir, comp), usecols=(1), unpack=True)
    ...
    def grab_frame_data(self, dataname, start, stop, molec=2, occupancy=False, flatten=False, average=False):
        ...
        if occupancy:
            ...
            for frame in range(start, stop):
                data1.append(self.frames[frame].grab_data(
                    dataname, occupancy=True)[0])
                data2.append(self.frames[frame].grab_data(
                    dataname, occupancy=True)[1])
                data3.append(self.frames[frame].grab_data(
                    dataname, occupancy=True)[2])
                data4.append(self.frames[frame].grab_data(
                    dataname, occupancy=True)[3])
            if flatten:
                # flatten arrays
...
if __name__ == "__main__":
    data = calculation_data(dirframes=501, dirstart=1,
                            dirstop=100, ncomps=[180, 20], compno=[2, 6])
